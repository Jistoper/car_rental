...
Route::controller(CarController::class)->as('car.')->group(function(){
    ...
    Route::get('/maintenance-add-store', 'storeMtn')->name('storeMtn'); // maintenance store create (store data to database)
    Route::get('/maintenance-edit', 'mtnEditView')->name('mtnEdit'); // maintenance edit (view form)
    Route::get('/maintenance-edit-store', 'mtnEditStore')->name('mtnStoreEdit'); // maintenance store edit (store changes to database)
